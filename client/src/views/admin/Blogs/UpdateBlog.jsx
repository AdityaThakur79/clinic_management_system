...
import { MdSave, MdCancel, MdImage, MdAutoAwesome } from 'react-icons/md';
import { useGetBlogByIdQuery, useUpdateBlogMutation } from '../../../features/api/blogApi';
...
const UpdateBlog = () => {
  ...
  useEffect(() => {
    if (data?.blog) {
      ...
      // Set existing featured image preview
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append featured image only if a new one is selected
      if (featuredImage) {
    ...
    return (
      <Center h="400px">
        <VStack spacing={4}>
          ...
          <Button onClick={() => navigate('/admin/blogs')}>Back to Blogs</Button>
        </VStack>
      </Center>
    );
  }
          ...
          <Grid templateColumns={{ base: '1fr', lg: '2fr 1fr' }} gap={6}>
            ...
            <GridItem>
              <VStack spacing={6} align="stretch">
                ...
                <Card bg={cardBg} borderColor={borderColor}>
                  <CardHeader>
                    <Text fontSize="lg" fontWeight="semibold">Blog Statistics</Text>
                  </CardHeader>
                  <CardBody>
                    <VStack spacing={3} align="stretch">
                      <HStack justify="space-between">
                        <Text fontSize="sm" color={textColor}>Views</Text>
                        <Badge colorScheme="blue">{data.blog.views || 0}</Badge>
                      </HStack>
                      <HStack justify="space-between">
                        <Text fontSize="sm" color={textColor}>Likes</Text>
                        <Badge colorScheme="green">{data.blog.likes || 0}</Badge>
                      </HStack>
                      <HStack justify="space-between">
                        <Text fontSize="sm" color={textColor}>Reading Time</Text>
                        <Badge colorScheme="purple">{data.blog.readingTime || 1} min</Badge>
                      </HStack>
                      <HStack justify="space-between">
                        <Text fontSize="sm" color={textColor}>Created</Text>
                        <Text fontSize="sm">{new Date(data.blog.createdAt).toLocaleDateString()}</Text>
                      </HStack>
                      {data.blog.publishedAt && (
                        <HStack justify="space-between">
                          <Text fontSize="sm" color={textColor}>Published</Text>
                          <Text fontSize="sm">{new Date(data.blog.publishedAt).toLocaleDateString()}</Text>
                        </HStack>
                      )}
                    </VStack>
                  </CardBody>
