...
import {
  ...
  Spinner, Center, Alert, AlertIcon, AlertTitle, AlertDescription, Icon, Avatar, SimpleGrid
} from '@chakra-ui/react';
import { useGetTodayAppointmentsQuery, useUpdateAppointmentStatusMutation, useDeleteAppointmentMutation } from '../../../features/api/appointments';
...
import { MdAssignment, MdBusiness, MdPhone, MdEmail, MdPerson, MdSchedule, MdToday } from 'react-icons/md';
...
const TodayAppointments = () => {
  ...
  // API hooks
  const { data, isFetching, refetch, error, isLoading } = useGetTodayAppointmentsQuery({
        ...
        <SimpleGrid columns={{ base: 1, md: 4 }} spacing={4}>
          <Card bg={cardBg} borderColor={borderColor}>
            <CardBody textAlign="center">
              <Text fontSize="2xl" fontWeight="bold" color="blue.500">{data?.total || 0}</Text>
              <Text fontSize="sm" color="gray.600">Total Appointments</Text>
            </CardBody>
          </Card>
          <Card bg={cardBg} borderColor={borderColor}>
            <CardBody textAlign="center">
              <Text fontSize="2xl" fontWeight="bold" color="blue.500">{data?.grouped?.booked?.length || 0}</Text>
              <Text fontSize="sm" color="gray.600">Booked</Text>
            </CardBody>
          </Card>
          <Card bg={cardBg} borderColor={borderColor}>
            <CardBody textAlign="center">
              <Text fontSize="2xl" fontWeight="bold" color="green.500">{data?.grouped?.completed?.length || 0}</Text>
              <Text fontSize="sm" color="gray.600">Completed</Text>
            </CardBody>
          </Card>
          <Card bg={cardBg} borderColor={borderColor}>
            <CardBody textAlign="center">
              <Text fontSize="2xl" fontWeight="bold" color="red.500">{data?.grouped?.cancelled?.length || 0}</Text>
                  ...
                  <Tbody>
                    {filteredAppointments.map(appointment => (
                      <Tr key={appointment._id} _hover={{ bg: 'gray.50' }}>
                        ...
                        <Td>
                          <Text fontSize="sm" fontWeight="medium">{formatTime(appointment.timeSlot)}</Text>
                        </Td>
                        <Td>
                          <Select
                  ...
                  <CardBody pt={0}>
                    <VStack spacing={4} align="stretch">
                      ...
                      <Box>
                        ...
                        <Text fontWeight="medium">{formatTime(selectedAppointment?.timeSlot)}</Text>
                      </Box>
